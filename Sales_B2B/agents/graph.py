...
def extract_json(text):
    try:
        ...
        pass
    return {"score": 50, "rationale": "Parsing failed", "next_action": "Review"}
...
async def human_review_node(state: ProspectState):
    return {"needs_human_review": True, "rationale": f"FLAGGED FOR REVIEW: {state['rationale']}"}

def route_based_on_score(state: ProspectState):
